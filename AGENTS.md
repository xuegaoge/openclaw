...
**Implementation/Flow**: Local git remotes are structured as `upstream=https://github.com/openclaw/openclaw.git` and `fork=https://github.com/xuegaoge/openclaw.git`. `main` is the only long-lived branch. The local update source is always `upstream/main`: fetch official upstream, integrate or replay the user's custom commits onto that latest upstream state in a clean workspace when possible, then push the resulting `main` to `fork/main`. The server does **not** update from official upstream directly; it deploys from `fork/main`.
